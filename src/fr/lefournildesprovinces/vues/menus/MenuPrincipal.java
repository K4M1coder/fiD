		...
		if (this.comboBoxMenu == null) {
			...
			case "administrateur":
				choix = new String[] {
						...
						"MAGASIN (Création, Mise à jour ou suppression d'un magasin)",
			...
			this.comboBoxMenu.addActionListener(new ActionListener() {
				...
				public void actionPerformed(final ActionEvent arg0) {
					...
					try {
						MenuPrincipal.this.selection = MenuPrincipal.this.comboBoxMenu
								.getSelectedItem().toString();
						if (!selection.isEmpty()) {
							MenuPrincipal.this.lblValider.setVisible(true);
							MenuPrincipal.this.lblMerciDeSelectionner
									.setVisible(false);
						} else {
							MenuPrincipal.this.lblValider.setVisible(false);
							MenuPrincipal.this.lblMerciDeSelectionner
						...
						final AlerteSelection fenetre = new AlerteSelection(
								MenuPrincipal.this.interfaceActuelle, message);
			...
			this.comboBoxMenu.setCursor(Cursor
...
//			});
	...
	private JLabel getLienmagasin() {
		if (this.lienmagasin == null) {
			...
			this.lienmagasin.setCursor(Cursor
					.getPredefinedCursor(Cursor.HAND_CURSOR));
