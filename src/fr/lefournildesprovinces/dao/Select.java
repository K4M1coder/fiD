...
public class Select {
...

	{
		...
		try {
			...
			final String sql = "SELECT CLIENT.IDCLIENT, CIVILITECLIENT, NOMCLIENT, PRENOMCLIENT, ADRESSECLIENT, NOMVILLE, CODEPOSTALVILLE, DATENAISSANCECLIENT, NOMMAGASIN, MAILCLIENT, ABONNEMENTNEWSLETTERCLIENT, NUMEROCARTEDEFIDELITE,AGECLIENT,TELEPHONEFIXECLIENT,TELEPHONEPORTABLECLIENT FROM CLIENT INNER JOIN VILLE ON VILLE.IDVILLE=CLIENT.IDVILLE INNER JOIN FREQUENTER ON FREQUENTER.IDCLIENT=CLIENT.IDCLIENT INNER JOIN MAGASIN ON MAGASIN.IDMAGASIN=FREQUENTER.IDMAGASIN INNER JOIN CARTE_DE_FIDELITE ON CARTE_DE_FIDELITE.IDCLIENT=CLIENT.IDCLIENT WHERE NUMEROCARTEDEFIDELITE=?";
